...
The website and Apple TV assembly animations sample the approved silver colours directly, with their existing reveal motion. Static website logos no longer invert or recolour the artwork. The website favicon and Safari touch icon are resampled from the approved iOS artwork, including its black tile and rim lighting. The PNGs are opaque, and the ICO retains its 16, 32, 48 and 64 px sizes. Every page links the favicon and Apple touch icon. The website build gives changed icons new content-hashed paths and retains standard root URLs for browser fallback requests. Existing browser caches may take time to refresh.
