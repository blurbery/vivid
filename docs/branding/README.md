...
<table width="100%">
  ...
  <tbody>
    ...
    <tr><td>Website favicon</td><td><a href="../../website/public/favicon.svg">SVG</a>, <a href="../../website/public/favicon.png">512 × 512 PNG</a> and <a href="../../website/public/favicon.ico">16–256 px ICO fallback</a></td><td>Silver V on charcoal</td></tr>
    <tr><td>Safari favourites</td><td><a href="../../website/public/apple-touch-icon.png">Opaque 1024 × 1024 PNG</a></td><td>Full-square silver V on charcoal</td></tr>
  </tbody>
</table>
...
The website uses the approved Glide entrance: two SVG pieces slide into their exact final positions once, then remain still. Reduced Motion and image-load failures retain the static logo. iOS and Apple TV use the same Glide timing with vector PDF pieces. The website, iOS and tvOS reveals do not add a white brightness wash, and the lower logo stays assembled at the bottom of the page. Only the lower Get Vivid section contains a TestFlight link. The website uses tighter section spacing on desktop and mobile, with brighter secondary text and footer links for readability. Static website logos no longer invert or recolour the artwork. The Safari touch icon uses the approved full-square iOS artwork, rendered directly from the vector at 1024 × 1024 with an opaque background. Website favicons use the same opaque, full-square artwork, with charcoal reaching every edge and no baked-in rounded corners or border. The SVG stays sharp at any size, with a 512 px PNG and 16, 32, 48, 64, 128 and 256 px ICO fallbacks. Every page links the SVG and PNG favicons and Apple touch icon. The website build gives changed icons new content-hashed paths and retains standard root URLs for browser fallback requests. Existing browser caches may take time to refresh.
...
These source catalogs supply app icons in new builds. Installing a new build is required to see them; updating these assets does not publish an App Store build. Website favicons and the Safari touch icon reuse the approved iOS artwork; documentation marks retain the transparent silver logo.

iOS and Apple TV setup and login use the preserved-vector `VividMarkSilver` PDF through `VividLogoView`. Startup and full-screen loading use `VividGlideLogo`: two vector pieces glide into place over 1.2 seconds with a 0.144-second stagger, then remain assembled. Reduce Motion, unavailable pieces and interrupted entrances settle on the static logo. `VividStartupView` still waits for both the logo and content readiness. Cold launch keeps the completed logo visible while the destination fades in over 0.55 seconds, with a subtle reduction in logo size. Reduce Motion uses a 0.2-second fade without scaling. Profile interaction and accessibility remain gated until the handoff completes. The static fallback uses the same available canvas size as the animated logo. Top Shelf uses the same two-piece mark and charcoal background. Home and other root tab pages intentionally omit a separate top-left logo. Physical-device presentation and focus/parallax for this artwork still require verification. The updated catalogue, including the three preserved-vector logo PDFs, passed `actool` compilation for iOS 18.0 and tvOS 18.0. The shared logo and Glide views passed focused Swift type-checking against both SDKs. Sandbox warnings prevented Simulator service access, so these checks do not establish simulator or device appearance. Image checks cover file references, expected dimensions, opaque app icons, transparent marks and full-width backgrounds without an inset tile. The website asset build passed. Startup animation rendering and physical-device appearance remain unverified.

Provider selection uses the Silo, Emby and Jellyfin logos to identify the supported server types. Vivid keeps its own app icon, startup artwork and branding. All three provider cards open native setup on mobile and Apple TV; Jellyfin uses the same server/login panel with its own authentication core. App Store submission remains a separate integration step.
