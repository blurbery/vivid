...
The experimental tvOS discovery Home now mounts its vertical row containers eagerly, while horizontal card strips remain lazy. This comparison targets rapid returns through mixed landscape and portrait rows to spotlight. Card dimensions, spacing and controls are unchanged. Navigation smoothness, initial Home loading and memory use await Living Room testing.
