...
Cold startup shows Vivid's star-to-ribbon animation on black, with Reduce Motion and a static-logo fallback. The animation completes when the V forms; routing still resolves the account state, but does not wait for a full network population of Home. Cached Home content can then appear while refresh continues. Saved-account selection and PIN rules are described in [Server Connections](server-connections.md#saved-accounts-on-apple-tv-iphone-and-ipad).
...
The app changes published in `v0.12.5` (`cb8c9490`) were built and installed on an Apple TV 4K (3rd generation), tvOS 26.6, during development. I confirmed playback, display matching, Continue Watching episode selection, the updated episode cards and settings. These were local device builds, not TestFlight archives. Isolated checks cover TMDb authentication, trailer selection and caching, server-only similarity sampling/ranking, account isolation, and Continue Watching season priority. No measured performance benchmark is recorded, and tvOS validation does not establish iOS runtime behaviour.
