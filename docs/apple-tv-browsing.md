...
The experimental tvOS movie and series pages use Apple's [media catalog sample](https://developer.apple.com/documentation/swiftui/creating-a-tvos-media-catalog-app-in-swiftui) fold-snapping algorithm and material-gradient mask. The artwork remains fixed while the shelves scroll, and the logo moves from the lower hero to the top centre below the fold. Existing playback controls sit lower in the hero. Apple's sample licence is retained in the source. This is an adaptation of Apple's public sample, not the Apple TV app's private source.

Series episodes form one native horizontal shelf across seasons. Focusing a different season automatically scrolls to its first episode; clicking a season also jumps there. Returning focus to the already highlighted season preserves the current episode position. The season selector uses compact capsule buttons: focused buttons are white with black text, unfocused buttons use the Play button’s white text and 10% white translucent fill, with an 18% fill marking the selected season when focus moves away. Episode runtime and the watched tick share one compact artwork badge. Focusing an episode updates the selected season. The route warms remaining season metadata sequentially in nearby-season order, with only six stills per warmed season. Missing pages retain a Load Season action. Native borderless buttons provide episode artwork focus; cards include episode number, title, summary and date, with watched/resume and context actions retained. The old series page-scroll coordinator, viewport lock, season pager, transition timers and handoff helpers have been removed. Separate episode and season detail pages retain their existing layouts.
...
Movie and series pages retain the credit line, playback readouts and action row within the lower hero. Separate episode pages retain their existing positions. The loading screen still uses the earlier hero framing. Twelve-point gaps separate credits, readouts and actions. Plots wrap within the action row width; Read more opens a separate scrollable Description panel. Start Over remains in the More menu. When the primary movie or series action says Resume, long-pressing it also opens a native menu with Start from Beginning. It uses the existing playback action and selected file without adding another button to the row.
