...
Focused native Top Shelf request/mapping checks and the tvOS extension build passed. A read-only Jellyfin check returned both rows and a credential-free series poster URL returned HTTP 200. Jellyfin Top Shelf presentation and interactions on the Apple TV Home Screen still need device verification. These changes have not been uploaded.
