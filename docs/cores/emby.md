...
Apple TV More Like This uses Emby’s native similar-items endpoint and retains results in the existing account-scoped cache. Emby item artwork shares in-flight image bytes across display, crop and palette sizes, with one retry for a timeout or lost connection; prefetching does not retry HTTP failures. On iOS and tvOS, the shared artwork view adds up to three recovery attempts after 2, 5 and 15 seconds for temporary transport failures and HTTP 408/500/502/503/504 responses. Each round retains the existing single transport retry, for at most eight transport attempts or four HTTP attempts per uninterrupted view task. This applies to Silo, Emby and Jellyfin. Missing artwork, authentication errors, rate limits, decoding failures and cancellation are not retried by this recovery policy. Cancelling the view task stops its recovery, and successful cached images retain their existing behaviour. A longer outage can still leave placeholders until the view loads again. Recommendation paths are unchanged.
...
<table width="100%">
  ...
  <tbody>
    ...
    <tr><td>Home rows</td><td>Continue Watching, Next Up, library-specific latest media, Collections and Recently Released Movies have legacy mappings. No fabricated Spotlight or generic Recently Added row is added. Those old rows are also excluded from the Emby cache; cached hero slides cannot recreate removed feed rows. Both Home API paths skip item requests for locally hidden rows unless Spotlight or the visible combined Continue Watching row needs them. Skipped row names and IDs remain available in Settings so the rows can be enabled again.</td></tr>
    <tr><td>Mobile hero</td><td>Emby uses the shared mobile spotlight, including its artwork treatment and metadata line. Cached Home metadata supplies classification, year, genre and rating when available, including parent-series metadata for episode slides.</td></tr>
    ...
    <tr><td>Seasons</td><td>Season queries are nonrecursive and accept only Season records. Known season numbers display as Season 1, Season 2 and so on; zero displays as Specials. Custom Emby names such as Reacher’s Gone Tomorrow no longer replace the numbered label. Missing/invalid season numbers retain the supplied name. This is an Emby-only client mapping; server metadata and Silo naming are unchanged. Each episode request is restricted to one season; iOS also warms other season lists for later navigation. Counts fall back to ChildCount when RecursiveItemCount is absent.</td></tr>
...
TMDb remains an optional personal credential stored in Keychain. Emby provider IDs supply metadata identity; when a TMDb ID is absent, the Emby path can resolve an IMDb ID through TMDb. IntroDB uses the series IMDb ID; TheIntroDB prefers TMDb and falls back to IMDb. Both use season/episode numbers through the shared Intro & Credit Skipper toggle and separate automatic-skip switches. Live Emby playback with these integrations still needs verification.

Original-file downloads use the existing download manager with native Emby authentication and the server's download policy. Their detail-page progress ring, poster artwork and Downloads cards use the shared mobile presentation. On iPhone and iPad, smaller downloads additionally use Emby's native conversion jobs when the account allows downloading and sync transcoding, and the service advertises conversion options for this device. Emby's [Downloads and Sync feature](https://emby.media/support/articles/Sync-Introduction.html) requires Emby Premiere. An unavailable conversion service leaves original downloads available to accounts with download permission. Season batches and monitoring remain unsupported by the Emby adapter.

Converted downloads offer 20, 10, 5, 2 and 1 Mbps caps. Emby chooses the source version and prepares a complete file before Vivid starts its existing background transfer. Vivid keeps the converted file's metadata, retries transfer acknowledgements and retains pending cleanup while offline, without restoring deleted downloads. Playback transcoding permissions alone do not grant download-conversion permission. These controls are mobile-only. Native routes and original-file byte ranges were checked against Emby 4.9.5.0; complete converted downloads, server cleanup and offline playback still require device verification with an eligible account.

The watchlist is stored locally in Vivid and can sync additions and removals through the optional MDBList connection. MDBList exports newly completed or explicitly marked watches after Emby confirms the write. It does not import watched history or override Emby resume positions. Eligible shared preferences and plugin credentials use the private iCloud vault, while native favourites and playback progress use Emby. See [plugin behaviour and import safeguards](../server-connections.md#plugins-on-iphone-ipad-and-apple-tv). Collection editing, realtime events and other unmapped features must not be advertised as complete.

In General → Home Screen, **Combine Next Up with Continue Watching** puts Emby resume items first, followed by Next Up episodes without duplicate content IDs. The combined row is titled Continue Watching and replaces the separate Next Up row while enabled. This Vivid setting is saved per server/profile, participates in the existing private iCloud preference sync, defaults to off and is available for Emby on iPhone, iPad and Apple TV. Jellyfin has a separate provider-scoped preference for the same combined-row behaviour. The combined row retains the Continue Watching row’s identity and position when that row exists. If the optional Next Up request fails, loaded Home rows remain available; cancellation and account changes still stop the request. Turning it off restores Emby’s usual rows. It does not change server watch history or resume positions.

On iPhone, iPad and Apple TV, Emby Movies and Series filter options come from Emby’s `/Genres` and `/OfficialRatings` endpoints, scoped to the selected library. These routes are confirmed in Emby 4.9.5.0’s own API schema; the obsolete `/Items/Filters` routes return 404 on that version. Genre, content rating, decade and watch status use native Emby queries; mixed libraries also offer media type. Different filter categories are combined with AND, with multiple values within a category matched with OR. Unsupported technical facets and the Match Any switch are not offered on this Emby path. A–Z uses Emby’s native name-prefix query, with # using its pre-A sort range and All removing the prefix. Saved unsupported Emby facets are cleared when restored. Filter-option requests stop after 10,000 rows or 100 pages per endpoint, and iOS offers Retry in the filter menu after a loading failure. Silo filtering retains its existing behaviour. On 12 September 2026, the owner confirmed that both Movies and Series filter menus opened and genre filtering worked on Apple TV. Both platform builds passed, and the owner subsequently confirmed that iOS and tvOS device tests passed.
...
On 8 September 2026, read-only inspection confirmed that an Emby season had a custom title and season number 4. The adapter now labels it Season 4 on both platforms without altering the server. Four source-derived mapping checks passed for numbered seasons, Specials, missing numbers and unchanged episode titles. The existing season regression test includes this custom-title case. Both device builds passed, were installed, and device checks confirmed the completed update on iPhone and Apple TV. This does not expand the untested integration and format coverage below.

On 13 September 2026, intermittent missing artwork was reproduced on Apple TV and reported in other Emby clients. Read-only server checks confirmed that three affected cached item references matched the Emby database, while their local image endpoints timed out both with and without cache tags. Sampled source image files remained readable. The internal server cause is unconfirmed; the tvOS request coalescing and bounded retry changes do not establish that this server issue is fixed. No server configuration was changed.
...
- [EmbyDownloads](../../iosApp/iosApp/Downloads/EmbyDownloads.swift): original-file downloads, conversion jobs and offline manifests.
