...
Normal launches now default to decoded PCM, including after a restart. Native JOC output requires the debug launch argument `-VividJOCNativeTrial`; `-VividJOCPCMControl` overrides it, and recovery can still disable native output. The previous process-only PCM opt-out silently reverted to native audio after the tvOS upgrade, confirmed by the post-update trace. This policy change keeps PCM testing consistent; it does not establish that audible sync is fixed.

### Timestamped PCM output trial

The owner-directed VLC-method trial replaces the tvOS PCM `AVAudioEngine` output with `VividPCMSampleBufferOutput`, using `AVSampleBufferAudioRenderer` and its synchroniser. The scheduling reference is [VLC's HomePod startup correction](https://github.com/videolan/vlc/commit/b0d5b12e07daa41b8d17d32107c11dad4e694cec). The adapter enqueues the first decoded PCM sample before starting at its source timestamp, retains queued samples and the timeline during pause, and re-anchors after flush to the first new sample. KSPlayer supplies playback admission rather than VLC's future host deadline. Video retains KSPlayer's existing renderer and follows the synchroniser's reported audio clock; output latency is not separately subtracted from that timeline.

`audio_pcm_output`, `audio_pcm_anchor` and `audio_pcm_timing` identify the backend, first-sample admission, queued endpoint and effective rate. Renderer failure or an automatic renderer flush stops playback with an error rather than silently reporting a healthy clock. The timer and readiness callback share one serial queue, with bounded frame admission and a retry when KSPlayer's decoded queue was temporarily empty. This is a PCM trial, not evidence of native Atmos or measured HomePod lip sync. Native Atmos work remains paused pending playback, pause/resume and seek validation.

`scripts/tests/pcm-samplebuffer-tests.py <task-work-directory>` runs the actual adapter with renderer/source doubles. Its 14 lifecycle checks passed for empty startup, first-frame retention, pause/resume, post-seek anchors, renderer backpressure, late decoded data, speed changes, failure and teardown. These checks do not execute Apple's renderer or measure acoustic synchronisation. The signed tvOS 27 build and signature verification passed. The update was installed in place and launched on Living Room; HomePod playback validation remains pending.

The first device trial produced no audible audio despite a running synchroniser and accepted samples. The owner confirmed quick startup. The correction packs KSPlayer's decoded channel planes as interleaved Float32 PCM, matching VLC's output representation, while retaining KSPlayer's sample counts, source timestamps, decoding and playback admission. It adds no startup prebuffer or fixed delay. `audio_pcm_signal` reports packed signal peak, volume and mute state. An additional 36 checks using real CoreMedia buffers passed for mono, stereo and 5.1 channel order, planar/interleaved input, 44.1/48 kHz rates, source timestamps, buffer interpretation and invalid input rejection. The signed correction built successfully and was installed in place and launched on Living Room. The owner confirmed audio is restored. The follow-up trace confirms planar input packed to interleaved stereo Float32 at 48 kHz, non-zero signal, volume 1, mute disabled, effective rate 1 and no PCM renderer failures in the captured session. Playback started in approximately 3.8 seconds in that trace. This verifies audible output, not final lip-sync accuracy. The owner subsequently confirmed playback remains out of sync. The approximately three-minute trace has a median KSPlayer scheduling gap of -35.8 ms and about 132.6 seconds of audio queued ahead at its end, with no PCM renderer failures. These counters do not measure acoustic delay. The PCM path still uses KSPlayer's immediate video presentation and scheduler; VLC's host-deadline admission and wider master-clock integration have not been reproduced. That adapter was a partial VLC-method adaptation, not a verified synchronisation fix.

The current PCM correction attaches KSPlayer's existing display layer and the audio renderer to one `AVSampleBufferRenderSynchronizer`, following [Apple's audio-driven timebase guidance](https://developer.apple.com/documentation/avfoundation/avsamplebufferdisplaylayer/controltimebase) and [VLC's audio-master clock principle](https://github.com/videolan/vlc/blob/master/doc/clock.md). Video keeps source timestamps and disables immediate display. KSPlayer admits up to two frames ahead so Apple can schedule presentation; it no longer uses the immediate-display catch-up policy for this connected PCM display-layer path. Other display paths retain their scheduler. Pause retains both queues on the stopped clock; seek flushes old video and the first new audio sample establishes the new anchor.

Audio admission is capped at the normal forward-buffer target (at least three seconds, scaled for faster playback), with at most one decoded-frame overshoot. This is an upper bound, not a startup prebuffer. The trace reports `audio_video_timeline_connected` and `audio_pcm_queue` with the attachment and queue lead. On this path `av_schedule_gap_ms` measures video submission lead relative to the shared clock, not final display or acoustic error. The signed tvOS build passed alongside 27 adapter lifecycle checks, 36 real CoreMedia packing checks, 29 scheduler integration checks and 3 checks with real Apple renderers sharing a paused timeline. The real-renderer check requires access to macOS media services; it plays no sound. The correction was installed in place and launched on Living Room. Its device trace confirms the shared-clock attachment, timestamped video, approximately three seconds of queued audio, normal rate, non-zero audio signal and no stalls through about two minutes. Startup was approximately 3.8 seconds. The owner subsequently confirmed that picture and sound stay in sync during playback and after pause/resume and seek on Living Room with HomePods. This is owner-verified playback on that device and route, not a measured acoustic offset or verification of other routes and formats. During longer viewing, the owner reported occasional very small perceived mismatches; no acoustic measurement established their cause. The captured run reached approximately 14:50 in the film with the shared clock connected, a 2.99–3.02-second audio queue, normal speed and no renderer failures. Its single buffering event occurred during a seek, with no later stalls in that capture. The owner subsequently reported perfect playback for a Bluey episode whose selected track was six-channel E-AC-3 without confirmed JOC, output as stereo PCM. The selected Punisher episode separately confirmed E-AC-3 JOC at stream and decoder level, also output as stereo PCM; native Atmos output remains unverified.

### Isolated native E-AC-3 trial after the PCM checkpoint

The working PCM source is preserved at `d013ba20bba3276e9b3d278ca8fd7598cf3ea053` on `ksplayer-trial`. The next owner-authorised Atmos trial selects a separate `VividNativeEAC3Output` only through the existing debug launch opt-in and confirmed JOC admission. The PCM renderer and buffer-packing implementation remain identical to that checkpoint.

The native adapter enqueues validated compressed EC-3 samples unchanged, caps audio admission at the normal forward-buffer target, and submits source-timestamped video up to two frames ahead of the shared Apple synchroniser. It uses one serial queue for audio lifecycle operations, retains queued samples during pause, and anchors the first new packet after seek. Renderer failures, unsupported packets, configuration changes and stalled playback retain the existing fresh-load recovery to PCM. Native speed changes continue to recover to PCM before changing rate.

`audio_native_queue` reports queue lead and video connection. `audio_native_rendering` reports the resolved `AVAudioSession.renderingMode` (5 means Dolby Atmos), supported AirPlay layouts and route type without device names. Source JOC, route capability, accepted packets and a running clock alone do not establish Atmos output. This follows [Dolby's native-decoding and output-verification guidance](https://professionalsupport.dolby.com/s/article/Dolby-Atmos-and-Dolby-Audio-over-AirPlay-Application-Developer-Guide).

Targeted validation passed: 31 native lifecycle/rejection checks, 31 scheduler checks, 32 EC-3 construction/classification checks, and the existing 66 PCM lifecycle, CoreMedia packing and real-Apple-renderer timeline checks. The native lifecycle tests use renderer/source doubles; the real-renderer check uses a paused timeline with no sound. The signed tvOS build and both trial/checkpoint signature checks passed, with bundle, team, Keychain and app groups unchanged. The trial was installed in place and launched on Living Room with `-VividJOCNativeTrial`. An installable copy of the PCM checkpoint is retained in the existing task work directory. Physical Atmos output, startup and sync remain pending on Living Room with HomePods using the confirmed-JOC Punisher episode.

The owner confirmed that the installed native trial is audible and in sync. Its first captured minute retained compressed JOC output without PCM recovery or renderer errors, with a roughly three-second audio queue and no stalls after startup. Apple reported rendering mode 0 throughout, so Atmos output remains unconfirmed. Apple's documentation also lists inactive/muted sessions and Now Playing ineligibility as reasons for this value; it is not a stereo-output verdict. A reporting-only follow-up records public session configuration, multichannel/spatial capability and the presence/rate of Now Playing metadata. A live debugger inspection did not complete reliably and coincided with an owner-reported freeze; the debugger was terminated and Vivid restarted. No further live debugger attachment is planned for this investigation.

The player statistics previously treated the Dolby stream's compatible base-layer format as an observed output conversion, producing an unsupported Dolby Vision-to-HDR10 arrow. At the owner's request, player statistics no longer use source-to-output conversion arrows. The dynamic-range row shows the configured rendering format, including `DV Profile 8.1` or `DV Profile 5` when the corresponding native Dolby decoder configuration succeeds, without claiming a measured HDMI display mode. On tvOS, the audio row reports PCM with the prepared renderer channel count for the decoded path or Apple's resolved rendering mode for native output, also using "Not reported" when unavailable; source JOC and source bitrate are not presented as verified output. The compact audio row omits renderer implementation details. These label changes do not alter decoding, display requests or playback timing.

The reporting follow-up found an active playback/movie session, seven Now Playing metadata keys with rate 1, and an advertised Atmos 5.1.2 layout, but `supportsMultichannelContent` and the route's spatial-enabled flag were both false. The pinned KSPlayer helper derives the content declaration from the route flag. The native-only correction declares multichannel support from the confirmed JOC content during preparation and restores the prior declaration on teardown, without changing channel-count preferences or PCM code. Its 33 native lifecycle/rejection checks passed, including content declaration and restoration. The subsequent live Punisher trial confirmed that declaration stayed true, original six-channel JOC input remained on native output, and Apple still returned rendering mode 0. The declaration alone did not resolve final Atmos reporting. Apple documents `renderingMode` as a read-only result, not an Atmos enable switch. Its [supported-layout documentation](https://developer.apple.com/documentation/avfaudio/avaudiosession/supportedoutputchannellayouts?changes=_5) says inactive or Now Playing-ineligible sessions return an empty list; the non-empty live list makes those explanations less likely. The owner reports the native trial sounds spatial and different from earlier PCM playback, but this is not a controlled same-scene comparison. No documented HomePod-specific trigger or proof that mode 0 disables Atmos has been established.
...
### Opt-in PCM channel preservation
