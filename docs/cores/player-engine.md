...
Both native routes expose their player-scoped Now Playing session. Local HLS permits Aether’s AirPlay host rewrite, while direct remote HLS with custom headers remains blocked for receiver fetching. This closes missing route integration; the reported startup pause still needs device verification.
