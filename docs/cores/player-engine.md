...
Vivid owns the shared audio session across episode changes. Its native driver uses `ao-avfoundation-manage-audio-session=no`, leaving activation and final deactivation to the app. This prevents an old player's asynchronous teardown, or a failed output probe, from deactivating a replacement player's session. The app also avoids reapplying an unchanged playback category, mode and route-sharing policy. Native channel negotiation remains enabled; standalone driver users retain session management by default.
