...
display-criteria reuse. Apple TV testing confirmed the earlier switch; other
...
The retained native patch enables a four-second PCM read-ahead limit for tvOS AirPlay. Refills wait when the renderer has at least one second queued and input cannot satisfy a complete read. A serial-queue timer drives bounded refills and is cancelled on pause, stop and teardown. On the first start after a seek/reset, the driver replaces the bounded PCM renderer and synchroniser, preserving volume and mute and rejecting delayed notifications from the old renderer. Explicit renderer failures request an audio reload. Ordinary pause/resume and EOF retain their sink. HDMI, iOS and compressed AC-3/E-AC-3 retain their existing output policy.

This requires the matching rebuilt native library, not only an app rebuild. The tvOS native workflow run `35690129461` and signed debug build passed for `c1a6aca`. On Apple TV 4K (3rd generation), tvOS 27.0, using HomePod gen 2 speakers, blurbery confirmed seeking with audio in sync after earlier candidates had reproduced buffering and seek failures. This is user-reported device validation; long sessions, other routes and a full provider matrix remain unverified. The [earlier observations](https://github.com/blurbery/vivid/blob/39a8f097ac3804a2fc503a9f0377e24f295afb15/docs/cores/player-engine.md#audio-support) remain available in source history.

Vivid owns the shared audio session across episode changes. Its native driver uses `ao-avfoundation-manage-audio-session=no`, leaving activation and final deactivation to the app. Final deactivation uses one shared teardown group to wait for retired native players from every player instance to finish and checks a per-load ownership token, so delayed cleanup cannot release a newer session. This prevents an old player's asynchronous teardown, or a failed output probe, from deactivating a replacement player's session. The app also avoids reapplying an unchanged playback category, mode and route-sharing policy. Native channel negotiation remains enabled; standalone driver users retain session management by default.

The ownership race is confirmed in code and covered by a shared-session test double. HomePod startup timing and long episode sequences still require physical-device validation; this change does not remove AirPlay's own buffering or prove the cause of every silent-audio report.
...
The [audio-driver workflow](../../.github/workflows/mpv-audio-driver.yml) applies that patch and rebuilds libmpv using the other pinned dependency binaries. Earlier workflow run 35028421151 produced iOS and tvOS slices with the EOF recovery fix only. Rebuild the current patch for the selected platform, then replace only the matching resolved artifact slice before linking: `ios-arm64/Libmpv.framework` for iOS or `tvos-arm64_arm64e/Libmpv.framework` for tvOS. Verify the `resuming compressed feed after audio EOF` marker and the native `manage-audio-session` option in the linked app binary. The option is required for the episode-handoff ownership fix; older rebuilt libraries do not include it. Verification uses the option name rather than additional session-ownership diagnostic logging.

The current source patch also bounds the first compressed-audio seek-to-start recovery attempt to the existing two-second grace after minimum priming. Incoming packets retain the slow-source extension before PCM fallback, but cannot keep postponing that first recovery attempt while the audio clock is stopped. The driver also defers an opportunistic audio read when its input queue cannot satisfy the request and AVPlayer retains at least the existing one-second priming reserve. This prevents prefetch from being reported as an output underrun and pausing the whole player. Reads resume as input arrives or the reserve drains, preserving real starvation and EOF handling. Buffer capacities and minimum priming are unchanged. Both changes have focused transport-decision coverage. In the local 0.14.3 (30) Apple TV build, blurbery confirmed that the prefetch change removed the pause after the switch. Earlier native artifacts do not contain it; iOS device validation and broader codec/route coverage remain outstanding.
...
The `mpv-experiment.yml` filename is historical: its workflow is named Lucid playback and still validates the current engine, package pins and platform builds. `PlaybackTrialTrace` records active playback diagnostics; it is not an alternative engine. Its bounded on-device file recording requires `VIVID_P8_TRIAL`, enabled by the committed tvOS Debug configuration and absent from the default Release configuration. Custom build flags must be checked on the actual archive.
...
On 16 September 2026, both signed device builds passed and were installed on iPhone and Apple TV. The linked binaries retained the audio EOF recovery marker. Twenty-six focused tests passed against the production language-selection logic. blurbery confirmed the installed playback and subtitle changes worked.

Earlier Apple TV checks covered seeking, synchronised playback and Dolby Vision with selected film and episode samples. These observations do not certify Atmos output, every codec or source, physical iPad behaviour, every PiP/AirPlay route or full external ASS styling. A successful build is separate from a device playback check.
