...
The [audio-driver workflow](../../.github/workflows/mpv-audio-driver.yml) applies that patch and rebuilds libmpv using the other pinned dependency binaries. Earlier workflow run 35028421151 produced iOS and tvOS slices with the EOF recovery fix only. Rebuild the current patch for the selected platform, then replace only the matching resolved artifact slice before linking: `ios-arm64/Libmpv.framework` for iOS or `tvos-arm64_arm64e/Libmpv.framework` for tvOS. Verify the markers `resuming compressed feed after audio EOF` and `audio session managed by application` in the linked app binary. The latter is required for the episode-handoff ownership fix; older rebuilt libraries do not include it.
