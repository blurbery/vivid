...
When tvOS playback starts with AirPlay selected, the PCM layout list permits 7.1, 5.1 and stereo, choosing the closest source layout rather than forcing every source to stereo or upmixing it to eight channels. DTS remains locally decoded; no DTS-to-AAC server conversion is requested. Other outputs retain `auto-safe`. The Apple output API can negotiate a different layout, so an eight-channel request does not prove eight-channel HomePod delivery. The debug trial records the actual player and audio-session channel counts, output latency, synchronisation and packet queues every two seconds. Living Room testing confirmed eight-channel player output but reproduced buffering and packet-queue exhaustion.

The native patch extends the existing macOS PCM read-ahead limiter to tvOS, disabled by default. Vivid enables a four-second limit for AirPlay at source load. Refills wait when the renderer retains at least one second and the input queue cannot supply a complete read, avoiding false underruns from opportunistic prefetch. Below that reserve, normal starvation and EOF handling remain active. The existing serial-queue timer is cancelled on pause, stop and teardown. HDMI, iOS and compressed AC-3/E-AC-3 paths retain their previous policy. Debug native events report renderer clock, fed position, queue lead, rate and status. The tvOS library from native workflow run 35687443564 was linked into the debug build at `856998a`. On Living Room Apple TV 4K (3rd generation), tvOS 27.0, blurbery confirmed smooth DTS 7.1 playback through HomePod gen 2 speakers. Roughly five minutes of diagnostics showed eight-channel player PCM, approximately four seconds of renderer read-ahead and no recorded fault events. blurbery subsequently confirmed that audio synchronisation was correct, with no perceived offset. Internal sync readings alone do not measure speaker-to-screen timing. This requires the rebuilt native library; an app-only build with the older library cannot exercise the limiter. Broader routes, track changes and long-session coverage remain unverified. A later long forward seek caused audio loss, frozen playback and an app exit. The follow-up candidate drives bounded tvOS PCM refills from a repeating serial-queue timer, retains the heartbeat during renderer backpressure, serialises pause/resume with refills and requests audio reload on an explicit renderer failure. Device testing of the timer follow-up still reproduced audio out of sync immediately after seeking, without a logged system restart. The next candidate replaces the bounded tvOS PCM renderer and synchroniser on the first start after reset, matching a fresh playback epoch. Renderer replacement preserves volume and mute, with user controls serialised on the replacement queue. Ordinary pause/resume and EOF retain their sink, delayed notifications from the old renderer are ignored, and allocation failure requests an audio reload without feeding the obsolete sink. The tvOS native workflow run 35690129461 and signed debug build passed for `c1a6aca`. On the same Living Room Apple TV and HomePod setup, blurbery confirmed that seeking now worked correctly with audio in sync. This is user-reported device validation; repeated long sessions, other routes and other providers remain unverified.
