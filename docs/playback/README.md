...
I confirmed Dune starts quickly and has sound when Vivid automatically chooses its working AC-3 alternate. On 9 September 2026, I also confirmed TrueHD 7.1 source playback on Apple TV through HomePod (2nd generation), using both Silo and Emby. The [player engine core](../cores/player-engine.md) records that result and distinguishes source playback from speaker output. Ten focused physical-iPhone tests previously passed for the audio preference and embedded-media path, including a non-contiguous audio stream ID and exactly one source open. Physical iPad coverage and broader surround-output verification remain separate checks.
...
Apple TV observes automatic audio-renderer flushes and output-configuration changes. It can replay retained, unplayed audio samples against the existing playback clock without restarting video or reopening the source. The replay queue is bounded to 16 MiB and 512 samples; a bounded seek/rebuild fallback remains when replay cannot recover. Output-latency-aware audio enqueueing and avoiding redundant synchronizer-rate assignments keep the tested HomePod startup continuous. This recovery is tvOS-only and does not change the iPhone audio route.

During development, 35 focused quality tests passed on iPhone 16 Pro Max (iOS 26.6.1), covering simulated buffering, one-time recovery, cancellation eligibility, rejected choices, local settings, legacy quality pairs, recovery requests and Emby negotiation. On 8 September 2026, the owner confirmed the completed changes work on that iPhone and Living Room Apple TV 4K (3rd generation, tvOS 26.6). The TV checks included normal E-AC-3 playback and the Harry Potter DTS file through HomePod (2nd generation); the owner also confirmed both platforms after the final buffer update. These are functional device results, not measured startup benchmarks or universal codec/output certification. Deliberately throttled-network fallback, physical iPad behaviour, DTS:X object preservation and other receiver layouts remain outside the recorded checks.
