...
The separate [HDMI recovery policy](../cores/player-engine.md#hdmi-audio) is opt-in for tvOS Debug builds only. It detects blocked audio delivery, permits one audio-only reset per load and skips overdue samples during catch-up. It does not replace the shared renderer or alter normal startup, decoding or display matching. The [mini-core guide](../cores/player-engine.md#mini-cores) defines these shared and route-specific responsibilities.
