...
Cast and crew pages use Home-sized filmography posters and the shared, left-aligned title/year caption settings. Cached person metadata appears on reopening while fresh metadata and filmography load concurrently. First-time missing metadata still depends on server enrichment.
