...
Detail pages place Cast & Crew, personal-TMDb Trailers and server-only More Like This before Media Information. The panels show the selected file’s available technical metadata, not invented values; series identify the selected episode. Video/file bitrates display in Mbps, with Silo’s kbps and Emby’s bps normalised before formatting. Audio retains kbps for sub-Mbps values. Movie/series artwork stays attached to the card’s top during fast scrolling; Home retains its separate parallax. Playback readouts remain compact equal-width glass pills. Long descriptions open separately; mobile movie and series pages have no extra More button. Downloads retain their progress rings and main-series artwork. Retry states use the shared glass treatment; cancelled or stale detail requests do not flash an error while closing.

The earlier VividKit migration checks are recorded through `v0.13.0` (`2ee9005c`). On 8 September 2026, the owner confirmed the completed Search/Settings transitions, detail/media-information layout, quality controls, audio/buffer fixes, Home state refresh, profile-deletion flow and Emby season labels on iPhone 16 Pro Max (iOS 26.6.1) and Living Room Apple TV 4K, 3rd generation (tvOS 26.6). Both were local development installs, version 0.6.0 (1), built from the source changes on `fix/search-details-playback-quality`; documentation was reconciled after that confirmation. Thirty-four focused Home-refresh/profile-policy tests also passed on the iPhone during this update. The owner’s functional confirmation is not an all-format playback certification, a benchmark or physical iPad coverage.
