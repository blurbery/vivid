...
The mobile app shares the TV feature owners, with touch layouts. Home, Libraries, For You, Downloads and Settings use black page backgrounds. Movie and series details retain their artwork-colour surface. The Downloads page uses glass storage and transfer cards, live byte and speed progress, and locally fetched movie or main-series posters. Mobile headings use 30-point rounded bold text and smaller section labels; TV headings stay at 40 points.
...
The signed-in account has a ring around its saved-account avatar on mobile and Apple TV. Saved account cards, login sessions and optional Vivid PIN records share a private iCloud vault across the three platforms. Sign Out clears the saved session while retaining its card; deleting a signed-out account records a tombstone so an older device cannot restore it. Downloads, preferences and metadata caches remain local. Home metadata is stored on disk per server/profile, with up to 20 items per row and 10 spotlight candidates. Metadata shows cache state and clear controls. Hiding a Home row removes its row-cache entry, while that section remains eligible for spotlight selection.

### Detail pages and validation

Detail pages place Cast & Crew, personal-TMDb Trailers and server-only More Like This before the divided Details panel. Playback readouts use compact equal-width glass pills with symbols, abbreviated version/audio information and subtitle language. Tap a long description to open it separately; mobile movie and series pages have no extra More button. The download action draws a progress ring while registering or transferring and returns to its completed state when finished. Series download sheets and their season/episode pickers use the same glass background, and episode, season and whole-series transfers keep the main series poster in Downloads. IntroDB, Seerr and metadata stores are shared with TV; shared source names beginning with TV do not imply a separate mobile copy.

The VividKit migration was built and installed on iPhone 16 Pro Max (iOS 26.6.1) and Living Room Apple TV 4K, 3rd generation (tvOS 26.6). I confirmed the Search keyboard no longer moves Home, player unlock follows physical orientation, and closing landscape playback leaves the detail page portrait without the player spinning. I also confirmed fast Dune playback after automatic compatible audio selection, account rings, the updated Home/detail episode indicators, the final iOS download fix and same-tap player controls. These app changes are included through `v0.13.0` (`2ee9005c`). Four focused iPhone tests passed for the final artwork URL and control-visibility paths. Live multi-device iCloud restore and deletion still need a separate two-device check. TV build/install/launch checks do not establish playback parity for all formats. No physical iPad verification is recorded; older layout checks below apply only to the revisions they tested.
