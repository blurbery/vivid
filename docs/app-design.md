...
Home uses the discovery spotlight and standard media rows described in [Apple TV Browsing](apple-tv-browsing.md). The experimental tvOS layout keeps the 580-point height, centres artwork, places the logo and metadata on the left, and blends blurred neighbouring artwork at the sides. Focus subtly brightens the ambient glow with no underline; the revised placement awaits device visual validation. Do not add a focus scale to the spotlight. Root tab pages use a slim glass bar with Search and Profile dividers, without a separate corner logo.
