...
Current trial source uses Lucid over public GPL KSPlayer on tvOS and VividKit on iOS, as described in the [player engine core](../cores/player-engine.md). The tvOS trial uses decoded PCM audio. HDMI output, AirPlay, HDR and Dolby Vision require source and route testing; earlier playback results do not validate this trial. Verify audible output and track switching on the selected build.
