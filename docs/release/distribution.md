...
4. Set the marketing version to the exact GitHub release and use the same TestFlight build number for iOS, tvOS and every embedded extension. Increment that shared build number for each new pair of uploads, including a new marketing version. Never reset it to 1. Check both finished archives before upload rather than relying on the Git tag alone.
