...
Release preparation on 12 September 2026 verified App Store Connect upload history at iPhone/iPad `18` and tvOS `17`, with no newer processing builds. The project baseline now prepares `0.14.3 (19)` for iPhone/iPad and `0.14.3 (18)` for tvOS. These contain the security fixes from PR #10, the approved Settings/profile refinements and eligible mobile Emby download conversion. Preparation alone does not mean an upload or tester availability; the completed distribution record is added after Apple confirms it.
