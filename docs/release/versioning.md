...
On 13 September 2026, tvOS-only `0.14.3 (21)` was archived from merged main commit `0d34aff0e2040233ce491f46af3d1f6c6e6c1631` using Xcode 26.6 with `CURRENT_PROJECT_VERSION=21`. App Store Connect showed tvOS build 20 before the upload. This includes PR #14; iOS was not rebuilt or uploaded. The archive passed app/Top Shelf version, signature and existing Keychain identity checks, and bundled privacy manifests and notices were reviewed. Apple accepted the upload at approximately 8:16 pm (Australia/Sydney). Processing completed, English (Australia) notes with immutable source and dependency links were saved, and both Internal Testers and External Testers showed Testing after beta submission with automatic notification enabled. Missing dSYMs for bundled media libraries did not block acceptance but limit their crash symbolication. Existing carousel and concurrency checks are recorded in PR #14; no new device playback test was performed on this distribution archive. The documented terminal-artwork rotation limitation and independent Emby server timeouts remain. The archive and app symbols are retained. This build-counter and distribution-record follow-up uses `[skip release]`.
...
These are historical records, not a live App Store Connect status feed. Pending checks refer to the stated revision and are not automatically satisfied by a later build. The latest recorded uploads are iPhone/iPad build 21 and tvOS build 21, as recorded above.
