...
Account transitions save already-verified outgoing account metadata without waiting for a server refresh. Once the viewing profile is authorised, Home opens from its scoped disk cache and refreshes in the background; an empty cache shows loading dots. Fresh Home and library requests do not hold the login animation open. Launch prefetch covers the active session, not every saved account, and the visible Home continues its ten-second refresh.
