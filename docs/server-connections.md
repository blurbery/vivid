...
<table width="100%">
  ...
  <tbody>
    ...
    <tr><td>Jellyfin</td><td>Native core implemented; see <a href="cores/jellyfin.md#verification">verification and limits</a></td></tr>
  </tbody>
</table>
...
Account updates during launch keep the current login-restoration attempt running. If active startup loading lasts more than two seconds, the logo shows Vivid’s loading dots underneath. Temporary Keychain failures use bounded retries; if the saved login still cannot be read, the existing Retry alert remains available and the dots stop while waiting for input.

On iPhone, iPad and Apple TV, Settings → Servers → Manage Servers → Add Server opens a separate Add Server page within Settings, with Silo, Emby and Jellyfin choices. Back returns to Manage Servers without resetting the current session. Each provider’s connection form also returns to those choices with Back. Apple TV retains its saved-account authentication form and uses the Siri Remote Back button for both steps, without additional on-screen Back buttons. Restore from iCloud remains on first-run setup and is not shown in this Add Server flow.

Settings lists circular saved-account cards and Add Profile. iPhone, iPad and Apple TV support up to four saved accounts. Add Profile is hidden at the limit. Here, a **profile card is a saved server account**, identified by server and user; it is distinct from a Silo viewing profile within that account. Add Profile signs in another Silo, Emby or Jellyfin account. The iPhone/iPad Who’s watching screen keeps one or two accounts and Add Profile on the original horizontal row. With three or more accounts it uses the fitted layout: three saved accounts across, with Add Profile centred underneath; four saved accounts use two rows of two. A ring highlights the signed-in account on both mobile and Apple TV. Selecting an inactive saved account switches sessions, while selecting the active card opens its account settings.

On Apple TV, native tvOS users retain separate server lists, login credentials and saved-account selections. Returning to a configured native user restores their selected local saved session before startup routing, without requiring iCloud. New native users still start setup. Migration only adopts legacy shared server entries referenced by that user's saved accounts or active server; it does not import another native user's credentials. Top Shelf reads the current native user's credentials too. It shows Continue Watching and Next Up for a single signed-in saved account when the existing profile/PIN policy allows it. Multiple saved accounts, signed-out accounts and locked profiles show static Vivid artwork instead. Account changes request a Top Shelf refresh; tvOS controls when the cached shelf is refreshed.
...
<table width="100%">
  ...
  <tbody>
    ...
    <tr><td>Remove Server</td><td>Remove the saved server and every linked profile from Vivid across iCloud devices, including their saved Home metadata and associated artwork. Offline devices apply the deletion on their next successful sync. Accounts and media on the actual server are unchanged.</td></tr>
...
Account metadata is stored locally and session tokens are kept in Keychain; the entered password is not retained. Startup retries temporary Keychain availability failures twice. A persistent read failure offers Retry without treating the account as signed out, and failed token reads are not cached. Restoring an account uses the HTTP identity-transition gate, cancels outgoing requests and shared artwork work, and clears active response/profile and decoded artwork caches. Both Apple TV and mobile rebuild the routed screen after the destination session is restored, including switches between accounts on the same server. Access and refresh tokens are captured together with the matching viewing-profile proof. A server can still revoke or expire a session, in which case fresh authentication may be necessary.

Artwork requests, decoded images, disk responses and Home snapshots are scoped by server, signed-in account and viewing profile. Saved account avatars use their own account scope even while another account is active. Older Home snapshots without account ownership are not reused; this does not remove saved sessions.

Account transitions save already-verified outgoing account metadata without waiting for a server refresh. Once the viewing profile is authorised, Home opens from its scoped disk cache and refreshes in the background; an empty cache shows loading dots. Fresh Home and library requests do not hold the login animation open. Launch prefetch covers the active session, not every saved account. On iPhone and iPad, the visible Home refreshes every ten seconds. On Apple TV, Silo Home refreshes every 30 minutes while visible; Jellyfin refreshes on foreground entry and every ten seconds after the previous visible-Home refresh completes. Emby uses entry and item-change refreshes.

On iPhone and iPad, tap a profile to select or edit it. Hold and drag to rearrange: the held card follows the finger and the other profiles move aside as it passes over them. Release within the profile row to save the previewed order; releasing outside the profiles cancels the move. VoiceOver also offers Move earlier and Move later actions. On Apple TV, hold a profile to pick it up, move left or right, then press centre to drop and save; Back cancels. Rearranging keeps the profile row at its normal height, without Done or delete buttons. Delete Profile is inside the profile’s account settings and still requires confirmation. Visible profile selector and Settings cards refresh iCloud every ten seconds while active and no move is in progress. Apple TV wobble honours Reduce Motion. These controls manage saved Vivid connections, not accounts on the media server.
...
Deleting a saved account on mobile or Apple TV writes a dated tombstone, whether the account is currently signed in or signed out. Every device applies that tombstone before uploading its local accounts, which prevents a stale iPhone, iPad or Apple TV from adding the deleted account back. The saved server remains when another saved account still references it. Deletion reaches another device at its next successful iCloud sync; an offline device may retain the card until then. Removing a server also records a server deletion, so profiles held only on another device are removed when they next sync. Only entering credentials again after the deletion can deliberately restore that same server/user identity. Removing the app does not delete the private CloudKit vault or accounts from the user’s other devices; it removes data held by that installation.

The encrypted private iCloud vault syncs saved accounts, sessions, optional Vivid PINs, profile order, shared browsing/navigation/metadata/download preferences and configured TMDb, Seerr, MDBList and OpenSubtitles credentials. Shared preferences and connections belong to the matching server account and viewing profile, not to every account using the same Apple iCloud login. A second Silo account has its own settings; an Emby account does not inherit a Silo account’s keys or settings. Plugin reads and edits pause while the active server/profile does not match the selected saved account. Unowned legacy global TMDb credentials and TV layout choices are not copied into a newly selected account. Playback and subtitle preferences, downloaded media and metadata/artwork caches remain device-local. Watched and resume state continues to sync through the selected media server’s own API. Detail-page watched/unwatched changes refresh Home after a successful write, with hidden Apple TV refreshes deferred until return. Visible, active iPhone/iPad Home reloads every ten seconds. Apple TV refreshes on entry when needed and after playback or watched-state changes; Silo additionally refreshes every thirty minutes while Home remains visible to renew signed artwork URLs. Jellyfin refreshes on each foreground entry and every ten seconds after the previous refresh completes while Home is visible. Hidden or inactive Home pages do not keep polling.
...
Use the same Apple iCloud account and the matching saved server account and viewing profile on each device. On Apple TV, check the **current Apple TV user**, not only the App Store account or default user. Vivid uses that user's private CloudKit database. The four-account limit applies on iPhone, iPad and Apple TV, including new accounts restored from iCloud. It limits saved accounts per device/native TV user, not the number of devices using iCloud. Existing saved accounts can still be updated at the limit; older installations with more than four accounts retain them but cannot add more.
...
The vault restores saved connections and shared settings, not local caches. MDBList confirmed export checkpoints survive connection failures on that installation. Watched history is not imported from MDBList. OpenSubtitles' bounded download cache lasts only for the app session; temporary player files are removed when playback ends. Restoring a key cannot recover files or checkpoints removed with an installation.
...
OpenSubtitles accepts a personal consumer API key in Plugins. It validates the key before saving it in this device’s Keychain, scoped to the server account and profile and synced through the encrypted private iCloud account vault. During playback, open Subtitles → Find on OpenSubtitles, choose a language code and search, then choose a matching release to download and use. Searches use the title and, for episodes, season and episode numbers. Search text is trimmed and lowercased to match the API’s canonical URLs; credential-bearing redirects are accepted only for the same HTTPS search endpoint. Failed searches distinguish missing configuration, network failures and HTTP response codes without displaying credentials or raw responses. Downloads use OpenSubtitles’ quota; this API-key-only connection does not include account login or extra subscription allowances. Files stay local and temporary, with no media-server upload. Repeat downloads reuse a session-memory cache capped at eight files and 10 MiB; replacing an entry does not create another copy, and account/key changes clear the cache. A failed repeat search keeps the previous results only for the same query, language, playback context and connection. Downloaded tracks and their selection are retained across successful replacement loads for the same item, including quality changes. Moving to another item or ending playback removes the temporary files. Live account downloads, subtitle timing and external playback routes still require device verification.

Connect validates the API key and shows Connected without waiting for watchlist work. Neither service's old watched history is scanned or imported. Movies and individual episodes completed or marked watched in Vivid are queued for MDBList after the media server confirms the watched write. Offline completions join this path after their queued server update succeeds. Pending sends retain their original timestamp across retries, and confirmed exports are deduplicated. Repeat plays are not exported as separate rewatches. Missing IDs and unconfirmed matches stay queued without blocking later items or watchlists. Vivid checks while active, normally every ten minutes; new watches and watchlist edits request an earlier check. Quota responses pause retries for an hour, including manual checks.

Watchlists sync movie and series additions and removals in both directions using the existing Vivid watchlist API. The first sync combines the lists; later removals win over conflicting additions. Only titles available on the active server and profile are imported, with exact TMDb or IMDb matching. Search narrows candidates by title, so different server titles can leave an item unmatched. Progress also shows individual candidate checks. For Emby and Jellyfin, this syncs Vivid’s account-scoped watchlist, not the server’s favourites. Missing, ambiguous or conflicting matches remain untouched and are retried later. An unavailable server item does not remove its MDBList entry. Completed writes are read back before advancing the local sync baseline. Each pass changes at most 32 titles. In-app watchlist edits request another foreground check; Sync now runs a manual check. Watchlist changes never call watched-state or playback APIs.

The API key stays in Keychain and syncs through the encrypted private iCloud account vault, like Trailers. Connection changes reach devices using the same iCloud account and saved server account/profile at their next successful vault sync. MDBList and OpenSubtitles save explicit connection changes in the encrypted sync ledger immediately. A missing local key does not count as a disconnect, and a successful vault sync restores the saved key even when its ledger entry has not changed. Explicit Disconnect still removes the key across synced devices. Sync checkpoints are local and account/profile-scoped. Interrupted syncs save confirmed export progress for retry. Watchlist reconciliation still reads fresh remote watchlists. Already-confirmed exports are skipped and stored identifiers are deduplicated. The key, external media identifiers, watched dates and watchlist changes are sent to MDBList, never media-server credentials. Disconnect removes the key and stops subsequent sync work without deleting history.
...
The shared plugin UI is available with all three server providers. Jellyfin routes watchlist and watched writes through its own [native core](cores/jellyfin.md); live Jellyfin plugin-account scenarios still need verification. The owner reported testing the preceding plugin build on iPhone and Apple TV. Follow-up fixes in [PR #8](https://github.com/blurbery/vivid/pull/8) passed targeted checks, signed device builds and remote iOS tests/tvOS build checks; those follow-up builds were not installed. This does not establish every live account, quota, subtitle or cross-device scenario.
...
These shared files retain Silo behaviour and dispatch to the active provider’s adapter. Provider-specific ownership is listed in the [Emby core](cores/emby.md) and [Jellyfin core](cores/jellyfin.md).
...
The compatibility work follows the Silo v2 contract deployed from `dd8c474b40d1cc485bf37d3117f7c148a891d4a6`. Cast and crew identifiers remain strings, watch-state timing fields map to Vivid's existing models, and batched settings use repeated query parameters. Playback mutation IDs use the server's canonical UUID format, and related-title queries retain a consistent page size within each catalogue window. Live tvOS checks cover profile selection, title and Spotlight details, and playback; they do not establish exhaustive settings, downloads or server-upgrade coverage. Vivid does not deploy, administer or modify Silo as part of connecting to it.

The server's display name labels that connection. It must not replace Vivid's app identity. Authentication and profile flows remain provider-specific behind the shared sign-in form unless verified otherwise.
...
The progress request fields match `blurbery/silo-server` at `d91fd15194d126cf67c85db1e908ddb163fee62d`. This is a source-contract check, not an end-to-end device test of that server revision. Preserve the existing client reporting when changing branding, and keep Silo, Emby and Jellyfin reporting separate when extending any provider.
