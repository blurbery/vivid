...
Settings → Plugins appears immediately below Servers. Trailers uses the existing personal TMDb connection and settings. MDBList accepts a personal API key from [MDBList Preferences](https://mdblist.com/preferences/). A validated connection stays connected when history is empty or a later sync fails; sync messages appear separately. MDBList and Trailers show a green dot beside Connected.

OpenSubtitles accepts a personal consumer API key in Plugins. It validates the key before saving it in this device’s Keychain, scoped to the server account and profile and synced through the encrypted private iCloud account vault. During playback, open Subtitles → Find on OpenSubtitles, choose a language code and search, then choose a matching release to download and use. Searches use the title and, for episodes, season and episode numbers. Search text is trimmed and lowercased to match the API’s canonical URLs; credential-bearing redirects are accepted only for the same HTTPS search endpoint. Failed searches distinguish missing configuration, network failures and HTTP response codes without displaying credentials or raw responses. Downloads use OpenSubtitles’ quota; this API-key-only connection does not include account login or extra subscription allowances. Files stay local and temporary, with no media-server upload. Replacing the media load can require downloading again. Live account downloads, subtitle timing and external playback routes still require device verification.

The current local implementation exports completed movie and individual-episode watched status from the selected server account and viewing profile. It checks while Vivid is active, normally every ten minutes, and backs off on errors or quota limits. The connection page shows four sync steps, with a progress bar and processed item counts when the server supplies a total. Fetches with an unknown total use an activity indicator. Watchlist batches with remaining changes are reported as incomplete batches rather than a fully finished sync. A complete MDBList snapshot is read before exporting. Completed items upload in batches of up to 50; unresolved items do not stop later batches and only confirmed results advance the checkpoint. Unconfirmed items keep their original timestamp for retry. Existing remote watches are acknowledged without another play; uncertain writes retain their original submitted timestamp for retries. When a source date is unavailable, the export uses the date Vivid first observes the completed item. Repeat plays are not exported as separate rewatches. Unsupported or missing external IDs are left unmatched.
