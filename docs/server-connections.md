...
On iPhone and iPad, Settings → Servers → Manage Servers → Add Server opens a separate Add Server page within Settings, with Silo, Emby and Jellyfin choices. Back returns to Manage Servers without resetting the current session. Each provider’s address form also returns to those choices with Back. Restore from iCloud remains on first-run setup and is not shown in this Add Server flow.
