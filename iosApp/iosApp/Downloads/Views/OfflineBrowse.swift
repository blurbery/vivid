                ...
                ForEach(group.seasons) { season in
                    ...
                    Button {
                        ...
                        Text(season.isSpecials ? "Specials" : "Season \(season.seasonNumber)")
                            ...
                            .foregroundColor(isSelected ? .vividOnSurface : .vividSecondaryText)
                            .padding(.horizontal, 14)
                            .padding(.vertical, 8)
                            .background(
                                Capsule()
                                    .fill(isSelected ? Color.vividChromeSelectedFill : Color.vividChromeRestingFill)
                                    .overlay(
                                        Capsule().stroke(
                                            isSelected ? Color.vividChromeSelectedBorder : Color.vividChromeRestingBorder,
                                            lineWidth: 1
                                        )
                                    )
                            )
...
struct OfflineDownloadDetailView: View {
                    ...
                    VStack {
                        ...
                        GeometryReader { geo in
                            ZStack(alignment: .leading) {
                                Color.vividOnSurface.opacity(0.22)
                                Color.vividOnSurface.frame(width: geo.size.width * fraction)
                            }
                        }
                        .frame(height: 4)
                    }
    ...
    private func availabilityChip(_ record: DownloadRecord) -> some View {
        ...
        .background(
            Capsule()
                .fill(Color.vividChromeSelectedFill)
    ...
    private func facts(_ record: DownloadRecord) -> some View {
        VStack(spacing: 0) {
            ForEach(factRows(record), id: \.0) { key, value in
                ...
                .padding(.vertical, 11)
                Divider().overlay(Color.vividDivider)
            }
        }
        .overlay(Divider().overlay(Color.vividDivider), alignment: .top)
    }
