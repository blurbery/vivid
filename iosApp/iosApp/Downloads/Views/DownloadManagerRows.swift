...
struct DownloadActiveRow: View {
    ...
    @ViewBuilder private var progressRing: some View {
        ...
        case .downloading, .paused:
            ...
            Button(action: onPauseResume) {
                ZStack {
                    Circle().stroke(Color.vividOnSurface.opacity(0.15), lineWidth: 3)
                    Circle()
                        .trim(from: 0, to: max(0.02, record.progressFraction))
                        .stroke(
                            Color.vividOnSurface.opacity(paused ? 0.55 : 1),
        ...
        default:
            ProgressView().controlSize(.small).tint(.vividOnSurface)
        }
    }
}
        ...
        Button(action: onPlay) {
            HStack(spacing: 11) {
                ...
                Image(systemName: watched ? "checkmark" : "play.circle")
                    ...
                    .foregroundColor(watched ? .vividOnSurface.opacity(0.5) : .vividOnSurface)
            }
            .padding(.horizontal, 14)
            .padding(.vertical, 8)
            .contentShape(Rectangle())
