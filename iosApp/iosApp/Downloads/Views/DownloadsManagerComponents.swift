...
struct DownloadsStorageHeader: View {
    ...
    private func segment(width: CGFloat, opacity: Double) -> some View {
        Color.vividOnSurface.opacity(opacity).frame(width: max(0, width))
    }
        ...
        if bytes > 0 {
            HStack(spacing: 6) {
                RoundedRectangle(cornerRadius: 2)
                    .fill(Color.vividOnSurface.opacity(opacity))
...
struct DownloadSelectionCircle: View {
    ...
    var body: some View {
        ZStack {
            Circle()
                .fill(selected ? Color.vividOnSurface : Color.clear)
                .overlay(
                    Circle().stroke(
                        selected ? Color.vividOnSurface : Color.vividOnSurface.opacity(0.35),
                        lineWidth: 1.5
                    )
                )
            if selected {
