...
enum VividMediaType {
    ...
    static func isSupportedLibrary(_ type: String) -> Bool {
        isMovieLibrary(type) || isSeries(type) || isMixedLibrary(type)
    }
    ...
    /// builds Home sections for every library type, so unsupported items must
    /// be stripped client-side before those libraries can leak rows into Home.
    static func isSupportedSectionItem(_ type: String) -> Bool {
        if isMovieLibrary(type) || isSeries(type) {
            return true
        }
        switch normalized(type) {
...
struct SeasonUserData: Codable, Hashable {
    ...
    init(played: Bool, episodeCount: Int) {
        self.played = played
        watchedCount = played ? episodeCount : 0
        unplayedCount = played ? 0 : episodeCount
        inProgressCount = 0
    }
...
struct Library: Codable, Identifiable, Hashable {
    ...
    var isMovieLibrary: Bool { VividMediaType.isMovieLibrary(type) }
    var isSeriesLibrary: Bool { VividMediaType.isSeries(type) }
    var isMixedLibrary: Bool { VividMediaType.isMixedLibrary(type) }
    var isSupportedLibrary: Bool { VividMediaType.isSupportedLibrary(type) }
...
/// Runtime-synthesized response from `VividAPI.libraryCollections`.
...
/// (see `Vivid/internal/api/handlers/playback.go::startPlaybackRequest`).
