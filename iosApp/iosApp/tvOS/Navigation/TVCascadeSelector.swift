...
struct TVCascadeSelector: View {
    ...
    private var twoLevelPanel: some View {
        ...
        // in the same place.
        HStack(alignment: .top, spacing: VividTheme.Skyline.flyoutGap) {
            librariesPanel

            flyout
                .frame(width: VividTheme.Skyline.flyoutWidth, alignment: .top)
                ...
                .animation(
                    reduceMotion ? nil : .easeInOut(duration: VividTheme.Skyline.flyoutOpenDuration),
    ...
    private var librariesPanel: some View {
        ...
        .padding(VividTheme.Skyline.dropdownPadding)
        .frame(width: VividTheme.Skyline.dropdownWidth, alignment: .leading)
        .modifier(TVSkylinePanelChrome(
            cornerRadius: VividTheme.Skyline.dropdownCornerRadius
        ))
    ...
    private var libraryRows: some View {
        ...
        if libraries.count > VividTheme.Skyline.cascadeMaxVisibleRows {
            ...
            .frame(maxHeight: estimatedRowHeight * CGFloat(VividTheme.Skyline.cascadeMaxVisibleRows))
        } else {
            VStack(alignment: .leading, spacing: 0) { rows }
        }
    }
    ...
    private var flyout: some View {
        if let anchorId = flyoutAnchorId,
           let library = libraries.first(where: { $0.id == anchorId }) {
            ...
            .padding(VividTheme.Skyline.flyoutPadding)
            .frame(width: VividTheme.Skyline.flyoutWidth, alignment: .leading)
            .modifier(TVSkylinePanelChrome(
                cornerRadius: VividTheme.Skyline.flyoutCornerRadius
            ))
            .fixedSize()
    ...
    private func panelHeader(_ text: String) -> some View {
        Text(text)
            .font(.system(size: VividTheme.Skyline.dropdownHeaderSize, design: .monospaced))
            .tracking(VividTheme.Skyline.dropdownHeaderSize * 0.26)
    ...
    private func flyoutHeader(_ text: String) -> some View {
        Text(text.uppercased())
            .font(.system(size: VividTheme.Skyline.flyoutHeaderSize, design: .monospaced))
            .tracking(VividTheme.Skyline.flyoutHeaderSize * 0.26)
            .foregroundStyle(Color.white.opacity(0.38))
            .lineLimit(1)
    ...
    private func scheduleFlyoutFollow(to id: Int) {
        ...
        flyoutFollowTask = Task { @MainActor in
            try? await Task.sleep(
                nanoseconds: VividTheme.Skyline.flyoutFollowDebounceMilliseconds * 1_000_000
            )
            ...
            if flyoutAnchorId != id {
                withAnimation(reduceMotion ? nil : .easeInOut(duration: VividTheme.Skyline.flyoutOpenDuration)) {
                    flyoutAnchorId = id
                }
            }
        }
    ...
    private var estimatedRowHeight: CGFloat {
        VividTheme.Skyline.cascadeRowTextSize
            + VividTheme.Skyline.cascadeRowPaddingVertical * 2
...
private struct TVCascadeLibraryRowLabel: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            Image(systemName: systemImage)
                ...
                .frame(width: VividTheme.Skyline.cascadeRowIconSize)

            Text(title)
                .font(.system(size: VividTheme.Skyline.cascadeRowTextSize, weight: .semibold))
                .lineLimit(1)
        ...
        .padding(.horizontal, VividTheme.Skyline.cascadeRowPaddingHorizontal)
        .padding(.vertical, VividTheme.Skyline.cascadeRowPaddingVertical)
        .background(
            RoundedRectangle(cornerRadius: VividTheme.Skyline.cascadeRowCornerRadius, style: .continuous)
    ...
    private var foreground: Color {
        isFocused ? .vividBackground : .white.opacity(0.9)
    }
}
...
private struct TVCascadeSectionRowLabel: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            Text(title)
                .font(.system(size: VividTheme.Skyline.flyoutRowTextSize, weight: .semibold))
                .lineLimit(1)

            Spacer(minLength: 0)
        }
        .foregroundStyle(isFocused ? Color.vividBackground : .white.opacity(0.86))
        .padding(.horizontal, VividTheme.Skyline.flyoutRowPaddingHorizontal)
        .padding(.vertical, VividTheme.Skyline.flyoutRowPaddingVertical)
        .background(
            RoundedRectangle(cornerRadius: VividTheme.Skyline.flyoutRowCornerRadius, style: .continuous)
                .fill(isFocused ? Color.white : Color.clear)
        )
        .focusEffectDisabled()
