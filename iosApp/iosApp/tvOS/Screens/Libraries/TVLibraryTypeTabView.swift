...
/// Series): the selected sub-destination's content, with the
...
struct TVLibraryTypeTabView: View {
    ...
    private func pillContent(for library: Library) -> some View {
        ...
        case .browse:
            TVLibraryGridView(
                ...
                topContentInset: VividTheme.Skyline.libraryContentTopInset,
                focusRequest: focusRequest,
                isTopMenuFocused: isTopMenuFocused,
    ...
    private var pillContentTransition: AnyTransition {
        ...
        return .asymmetric(
            insertion: .opacity.combined(with: .offset(y: VividTheme.Skyline.pillDriftY)),
            removal: .opacity
        )
    }
}
