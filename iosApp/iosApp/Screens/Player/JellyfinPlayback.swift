...
final class JellyfinPlayback {
    ...
    init(connection: JellyfinConnection, itemID: String, sourceID: String, playSessionID: String,
    ...
    static func prepare(metadata: Metadata, detail: WatchDetail, version: FileVersion, start: Double,
        ...
        guard let source = sources.first(where: { ($0["Id"] as? String).map(JellyfinAdapter.numberID) == version.fileId }),
              let sourceID = source["Id"] as? String else { throw JellyfinError.playbackUnavailable }
        ...
        var body: [String: Any] = ["UserId":connection.userID!, "MediaSourceId":sourceID, "StartTimeTicks":ticks(start),
            ...
            "DeviceProfile": ["Name":"Vivid", "MaxStreamingBitrate": (bitrateKbps ?? 200_000) * 1000,
        ...
        let info = try await connection.object("POST", "/Items/\(JellyfinConnection.id(detail.contentId))/PlaybackInfo", body: body)
        guard info["ErrorCode"] == nil || info["ErrorCode"] is NSNull,
              ...
              let playSessionID = info["PlaySessionId"] as? String else { throw JellyfinError.playbackUnavailable }
        let direct = playable["SupportsDirectPlay"] as? Bool == true
        let url: URL
        let method: String
        if direct {
            url = try JellyfinConnection.url(serverURL: connection.serverURL, path: "/Videos/\(JellyfinConnection.id(detail.contentId))/stream", query:["Static":"true", "MediaSourceId":sourceID, "PlaySessionId":playSessionID])
            method = "DirectPlay"
        } else if let rawURL = playable["TranscodingUrl"] as? String,
                  let base = URL(string: connection.serverURL.trimmingCharacters(in: CharacterSet(charactersIn:"/")) + "/"),
            ...
            components.queryItems = components.queryItems?.filter { !["api_key", "x-emby-token", "token", "access_token", "starttimeticks"].contains($0.name.lowercased()) }
            guard let resolved = components.url else { throw JellyfinError.invalidURL }
    ...
    func stopWithoutProgress() async throws {
        guard !stopped else { return }
        // Jellyfin treats a missing stop position as completion. Failed
        // retires an unqualified preview without saving its watch state.
        _ = try await connection.request("POST", "/Sessions/Playing/Stopped", body: [
            "ItemId":itemID, "MediaSourceId":sourceID,
            "PlaySessionId":playSessionID, "Failed":true
        ])
        stopped = true
    }
    ...
    func report(position: Double, isPaused: Bool, stopping: Bool = false) async throws {
        ...
        let report = stopping ? ["ItemId":itemID, "MediaSourceId":sourceID, "PlaySessionId":playSessionID, "PositionTicks":Self.ticks(lastPosition)] as [String:Any] : body
        _ = try await connection.request("POST", stopping ? "/Sessions/Playing/Stopped" : "/Sessions/Playing/Progress",body:report)
        if stopping {
            stopped = true
