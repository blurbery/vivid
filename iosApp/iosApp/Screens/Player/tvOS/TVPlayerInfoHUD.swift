...
///
/// Focus model (see docs/apple-tv-focus.md): every interactive row is a real
...
struct TVPlayerInfoHUD: View {
    ...
    private var panel: some View {
        Group {
            ...
            case .subtitles: SubtitlesPane(viewModel: viewModel)
        ...
        // translucent fill instead (see vividPlayerGlass).
...
private struct VideoPane: View {
    ...
    var body: some View {
        ...
        .animation(.easeOut(duration: VividTheme.fastDuration), value: activePicker?.id)
    }
...
private struct SubtitlesPane: View {
    ...
    /// a picker dialog or the appearance dialog closes.
    ...
    private var overlayActive: Bool {
        showAppearanceDialog || activePicker != nil
    }
    ...
    var body: some View {
        ...
        .animation(.easeOut(duration: VividTheme.fastDuration), value: showAppearanceDialog)
        .animation(.easeOut(duration: VividTheme.fastDuration), value: activePicker?.id)
    }
    ...
    private var optionRowsContent: some View {
        VStack(spacing: 2) {
            ...
            if viewModel.backendCapabilities.supportsSubtitleStyling {
                ...
                .focused($focusedOption, equals: .customAppearance)
                .id(Option.customAppearance)
                .disabled(viewModel.settings.subtitleMatchesSystemAppearance)
                HUDSettingRow(
                    label: "Size",
                    value: viewModel.settings.subtitleAppearance.fontSize.label
                ) {
                ...
                .id(Option.size)
                .disabled(viewModel.settings.subtitleMatchesSystemAppearance || !viewModel.settings.subtitleUsesDeviceAppearanceOverride)
                HUDSettingRow(
                    label: "Position",
                    value: viewModel.settings.subtitleAppearance.position.label
                ) {
