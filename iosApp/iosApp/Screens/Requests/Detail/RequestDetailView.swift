...
struct RequestDetailView: View {
        ...
        ZStack(alignment: .bottomLeading) {
            if let backdrop = RequestImageURL.build(detail.backdropPath, size: .backdrop) {
                AsyncImageView(url: backdrop, contentMode: .fill)
                    ...
                    .overlay(
                        LinearGradient(
                            colors: [.clear, Color.vividBackground.opacity(0.65), .vividBackground],
                            startPoint: .top,
                            endPoint: .bottom
                        )
                    )
            ...
            HStack(alignment: .bottom, spacing: VividTheme.padding) {
            ...
            HStack(spacing: 24) {
                ...
                if let message = viewModel.actionErrorMessage {
                    ...
                    Text("Reviewed by your server admin")
                        .font(.vividSmall)
                        .foregroundColor(.vividSecondaryText.opacity(0.7))
                }
            }
            .focusSection()
    ...
    private func primaryActionButton(_ detail: RequestMediaDetail) -> some View {
        ...
        Button {
            ...
            .background(
                Capsule().fill(action.isInteractive ? Color.vividOnSurface : Color.vividChromeRestingFill)
            )
            .overlay(
                Capsule().stroke(
                    action.isInteractive ? Color.clear : Color.vividChromeRestingBorder,
                    lineWidth: 1
                )
            )
        }
    ...
    private func buttonForeground(for action: RequestPrimaryAction) -> Color {
        action.isInteractive ? .vividBackground : .vividOnSurface
    }
