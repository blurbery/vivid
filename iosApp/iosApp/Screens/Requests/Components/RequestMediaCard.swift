...
struct RequestMediaCard: View {
    ...
    let onTap: () -> Void
    private var widthOverride: CGFloat? = nil
    @State private var uiCustomization = UICustomizationPreferences.shared

    init(result: RequestMediaResult, cardWidth: CGFloat? = nil, onTap: @escaping () -> Void) {
        self.widthOverride = cardWidth
    ...
    private var height: CGFloat {
        width * (VividTheme.posterCardHeight / VividTheme.posterCardWidth)
    }
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 22) {
            ...
            if widthOverride != nil || uiCustomization.cardPresentation.caption.showsTitle {
                caption
            }
        }
        .frame(width: width)
    ...
    private var posterPlaceholder: some View {
        ZStack {
            Rectangle().fill(Color.vividSurfaceElevated)
            Text(title)
                .font(.vividCaption)
                .fontWeight(.semibold)
