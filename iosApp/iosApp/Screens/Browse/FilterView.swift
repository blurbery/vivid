...
struct FilterView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                quickSection
                categoriesSection
                if !MediaServerProvider.active.usesNativeUser { matchSection }
                preserveSection
            }
            .listStyle(.plain)
    ...
    private func categoryRow(_ facet: CatalogFacet) -> some View {
        ...
        .filterListRowSeparatorTint(.vividDivider)
    }
    ...
    private var matchSection: some View {
        Section {
            HStack {
                ...
                .padding(2)
                .background(Capsule().fill(Color.vividSurfaceElevated))
            }
            .listRowBackground(Color.clear)
    ...
    private func matchOption(_ label: String, isOn: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(label)
                .font(.vividCaption)
                .foregroundColor(isOn ? .vividBackground : .vividSecondaryText)
                .padding(.horizontal, 14)
                .padding(.vertical, 6)
                .background(Capsule().fill(isOn ? Color.vividOnSurface : Color.clear))
        }
        .buttonStyle(.plain)
    }
            ...
            HStack {
                ...
                if isOn {
                    Image(systemName: "checkmark")
                        .font(.vividBody.weight(.semibold))
                        .foregroundColor(.vividOnSurface)
                }
            }
