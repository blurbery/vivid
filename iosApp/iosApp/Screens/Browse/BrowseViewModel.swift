...
class BrowseViewModel {
    ...
    func loadItems(reset: Bool = false) async {
        ...
        do {
            ...
            if reset && currentPage == 0 {
                ...
                response = try await VividAPI.shared.catalog(query: query)
            }
    ...
    func resultCount(for candidate: CatalogFilterState) async -> Int? {
        ...
        let response: CatalogResponse? = try? await VividAPI.shared.catalog(query: query)
    ...
    /// Refine the media family from the first loaded item without sending a
    /// `type` scope that could filter the page empty.
