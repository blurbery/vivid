                ...
                ForEach(LibraryPageTab.allCases) { tab in
                    Button {
                        ...
                        Text(tab.title)
                            ...
                            .foregroundColor(selectedTab == tab ? Color.vividBackground : .vividSecondaryText)
                            .padding(.horizontal, 16)
                            .padding(.vertical, 8)
                            .background(
                                Capsule()
                                    .fill(selectedTab == tab ? Color.vividOnSurface : Color.vividSurfaceElevated)
                            )
                    }
                    .buttonStyle(.plain)
                }
...
struct LibraryRecommendedView: View {
    ...
    private var content: some View {
        ScrollView(.vertical, showsIndicators: false) {
            LazyVStack(spacing: VividTheme.largePadding) {
