...
///
/// These deliberately diverge from `VividTheme` in a few places — the
...
enum HomeFeedMetrics {
    ...
    /// Screen gutter. Matches `VividTheme.safePadding` so variants stay
...
private struct HomeWatchedCheck: View {
    var body: some View {
        Image(systemName: "checkmark")
            ...
            .background(Circle().fill(Color(red: 0.16, green: 0.62, blue: 0.34)))
    }
}
...
struct HomePosterCard: View {
    ...
    private var height: CGFloat {
        (width * HomeFeedMetrics.posterAspect).rounded()
    }
...
struct HomeStillCard: View {
    ...
    private var secondLine: String? {
        guard isIOSResumeCard || showsEpisodeDetails, EpisodeCardCaption.isEpisode(item) else {
    ...
    var body: some View {
        HomeCardTap(
            ...
            VStack(alignment: .leading, spacing: 8) {
                artwork
                if showsCaption || isIOSResumeCard || showsEpisodeDetails { caption }
            }
            .frame(width: width, alignment: .leading)
        }
        .modifier(HomeCardMenu(
    ...
    private var isDirectlyPlayable: Bool { VividMediaType.isDirectlyPlayable(item.type) }
    ...
    private var caption: some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            if showsMetadata || isIOSResumeCard || showsEpisodeDetails {
            ...
            if showsMetadata, !isIOSResumeCard,
               (opensResumeContext && HorizontalMediaRailLayout.isPhone)
                ...
                Text(HomeFeedMeta.resumeCaption(for: item) ?? "0m left")
                    .font(.system(size: 11, weight: .regular))
                    .foregroundStyle(Color.vividOnSurface.opacity(0.55))
                    .lineLimit(1)
        ...
        HStack(spacing: 7) {
            if let icon {
                Image(systemName: icon)
                    .font(.system(size: 13, weight: .semibold))
                    .foregroundStyle(Color.vividOnSurface.opacity(0.85))
            }
            ...
            if let onSeeAll {
                Button(action: onSeeAll) {
                    Image(systemName: "chevron.right")
                        ...
                        .foregroundStyle(Color.vividOnSurface.opacity(0.35))
                }
                .buttonStyle(.plain)
            }
        }
