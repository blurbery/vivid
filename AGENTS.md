...
- Use `blurbery` as the public maintainer identity in documentation, templates and approval rules. Do not add a personal name, separate nickname or private contact details. Public validation records should describe hardware, OS, source revision and results without private device names, room labels, library/account identifiers or local upload times. Use `blurbery` with its GitHub noreply address for new commits; inspect public history separately because editing current files does not remove earlier disclosures. Do not rewrite history without explicit approval.
